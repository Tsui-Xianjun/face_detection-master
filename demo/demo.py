...
def parse_args():
    ...
    # parser.add_argument('--data', type=str, default='/home/cuixj/Desktop/faceimg',
    #                     help='The path of input and output file.')
    parser.add_argument('--data', type=str, default='/home/cuixj/Desktop/facevideo',
                        help='The path of input and output file.')
...
def main():
    ...
    if args.mode == 'image':
        ...
        frame_num=0
        for file_name in file_name_list:
            out_file = os.path.join(data_folder, file_name.replace('.mp4', '_v2_gpu_result111.mp4'))
            ...
            vid_writer = cv2.VideoWriter(out_file, \
                                         ...
                                         # cv2.VideoWriter_fourcc('F', 'M', 'P', '4'), 15, \
                                         cv2.VideoWriter_fourcc('X', '2', '6', '4'), 15, \
                                         (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), \
                                          int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
            while cv2.waitKey(1) < 0:
                frame_num = frame_num + 1
                print("current frame num is : %d \n" % frame_num)
                ret, frame = cap.read()
                if ret:
                    h, w, c = frame.shape
